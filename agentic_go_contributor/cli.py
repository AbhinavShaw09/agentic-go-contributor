...
def main(repo: str, issue: int, run_id: str) -> None:
    ...
    click.echo(f"   Dashboard: {_DASHBOARD_URL}/review/{run_id}\n")

    services = Services()
    services.review.init_run(run_id, repo, issue)

    try:
        graph = build_graph(services)
        final_state = graph.invoke(_initial_state(repo, issue, run_id))
    ...
    services.review.write_completed(run_id, final_state)
    click.echo(f"\n🔗 Dashboard: {_DASHBOARD_URL}/review/{run_id}")


def _initial_state(repo: str, issue: int, run_id: str) -> AgentState:
    return AgentState(
        run_id=run_id,
        repo_url=repo,
        issue_number=issue,
...
def _print_output(state: AgentState) -> None:
    ...
    if state.get("relevant_files"):
        click.echo(
            f"  ✓ Repository explored ({len(state['relevant_files'])} files, "
            f"{len(state.get('relevant_tests', []))} tests)"
        )
    if state.get("plan"):
...
def _save_results(state: AgentState) -> None:
    ...
    repo_slug = state.get("repo_url", "unknown").replace("/", "_").replace("https://github.com/", "")
    issue = state.get("issue_number", "?")
    run_dir = _RESULTS_DIR / f"{repo_slug}_issue-{issue}_{ts}"
